...
// ── Contra-trend hard gate ─────────────────────────────────────────────────
// Short-term indicators (1m RSI, MACD, delta5m) will temporarily look bearish
// inside a strong uptrend, and bullish inside a strong downtrend. That is noise.
// The regime and 15m HTF are the structural ground truth for a 15-min window.
// When both agree and the trade goes against them, apply escalating restrictions.
//
// This is exactly what caused the BUY NO loss at ADX 60 / TRENDING UP / 15m UP:
// the 1m model said DOWN, the structure said UP, structure was right.
//
...
const regimeDir        = regime?.direction;        // ‘UP’ | ‘DOWN’ | undefined
const adx              = regime?.adx ?? 0;

```
const regimeAgainst    = regimeTrending && regimeDir &&
  (regimeDir === 'UP') !== tradeIsUp;

const htfAgainst       = htfBias != null &&
  (htfBias.direction === 'UP') !== tradeIsUp;

if (regimeAgainst || htfAgainst) {
  const bothAgainst = regimeAgainst && htfAgainst;

  // Hard block: dominant trend + both structural indicators against
  if (bothAgainst && adx > 50) {
    console.warn(
      `[contra] Hard block — regime=${regimeDir} HTF=${htfBias?.direction} ` +
      `ADX=${adx.toFixed(1)} > 50, trade=${direction}. ` +
      `Dominant aligned trend. Contra-trade disqualified.`,
    );
    return {
      shouldTrade: false,
      direction:   null,
      ...
      signalScore,
      session:     session.name,
      reason:      `Contra-trend blocked — dominant ${regimeDir} trend (ADX=${adx.toFixed(1)}) with 15m HTF ${htfBias?.direction}. Both structural indicators oppose this trade.`,
      delta5m,
    };
  }

  // Score floor for weaker contra-trend conditions
  const contraScoreFloor = bothAgainst && adx > 35 ? 9
    : (bothAgainst || regimeAgainst)              ? 8
    : 8; // htfAgainst only

  if (signalScore < contraScoreFloor) {
    const who = bothAgainst
      ? `regime ${regimeDir} (ADX=${adx.toFixed(1)}) + HTF ${htfBias?.direction}`
      : regimeAgainst
        ? `regime ${regimeDir} (ADX=${adx.toFixed(1)})`
        : `HTF ${htfBias?.direction}`;

    console.warn(
      `[contra] Score floor — ${who} against ${direction}. ` +
      `score=${signalScore} < required=${contraScoreFloor}`,
    );
    return {
      shouldTrade: false,
      direction:   null,
